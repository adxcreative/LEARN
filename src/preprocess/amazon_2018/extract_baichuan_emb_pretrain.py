...
if __name__ == '__main__':
    ...
    with open('./amazon_2018/pretrain_data/product2item.dict', 'rb') as rf:
        product2text = pickle.load(rf)

    key_list = list(product2text.keys())
    print(f"total {len(key_list)} items")

    part_num = len(key_list) // 8 + 1
    cur_key_list = key_list[args.index * part_num: (args.index + 1) * part_num]
    ...
    # import pdb; pdb.set_trace()
    filepath = f'./amazon_2018/item_embeddings/part_{args.index}.txt'
    wf = open(filepath, 'w')

    with torch.no_grad():
        single_itemid_batch = []
        single_itemidinfo_batch = []
        for itemid in tqdm(cur_key_list):
